...
// Model load reads the whole GGML file (hundreds of MB) and builds the
// context — sync commands run on the Tauri main thread, which on Windows
// stops the message pump and flags the window "Not Responding" for the
// entire load (worst right after install: cold file + Defender scan).
...
// The warm-up runs a real inference — whisper.cpp pads the 1s of silence to a
// full 30s window — which takes minutes on low-end CPU-only machines. Keep it
