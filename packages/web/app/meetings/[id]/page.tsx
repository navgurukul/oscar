...
// Live public URL for a meeting — only when it's actually public. Built from the
// passed origin so the same helper works in click handlers (window.origin) and
// in render (origin held in state, empty until mount to avoid SSR mismatch).
function meetingShareUrl(
  ...
  return `${origin}/m/${meeting.publicShareToken}`;
}
...
export default function MeetingDetailPage() {
  ...
  const shareUrl = meetingShareUrl(meeting, origin);
...
function NotesTab({
  ...
  return (
    <section className="px-6 md:px-14 pt-9 pb-16">
      {shareUrl && (
        <div
          className="mb-9 flex items-center justify-between gap-4 rounded-2xl px-5 py-4 flex-wrap"
          style={{ background: v2.cream2, border: `1px solid ${v2.rule}` }}
        >
          <div className="min-w-0">
            <V2Caps color={v2.accent}>PUBLIC LINK · ANYONE CAN READ</V2Caps>
            <p
              className="mt-1 truncate"
              style={{ fontSize: 13, color: v2.inkSoft, maxWidth: 560 }}
            >
              {shareUrl}
            </p>
          </div>
