...
| Payments | Razorpay | — |
| STT | ONNX Runtime Web + speech-to-speech | whisper-rs (Metal on macOS; **CPU-only on Windows/Linux** — see GPU note below) |
...
- UI: [packages/desktop/public/pill.html](./packages/desktop/public/pill.html) — vanilla JS/CSS, Figtree font, Paper-pill tokens (white gradient, cyan-400 accent, cyan-500 toast). State machine: `rest → ready → expanded → recording → processing → inserted → rest` plus `error`, `copied`, `auth`, and `downloading` (shown when the hotkey/hover fires while the dictation model isn't ready yet — caption + live %, no audio captured; auto-collapses ~2.5s after progress stops). The host feeds it via the `pill-download-progress` event for the dictation role.
